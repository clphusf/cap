...
local function show_group_settingsmod(msg, data, target)
  ...
  local lock_leave = "no"
	...
	if data[tostring(msg.to.id)]['settings']['antilink'] == 'yes' then
	  ...
	  local antifuck = "no"
    if data[tostring(msg.to.id)]['settings']['antifuck'] then
        antifauck = data[tostring(msg.to.id)]['settings']['antifauck']
        end
...
local function lock_group_antifuck(msg, data, target)
  if not is_momod(msg) then
    return "For moderators only!"
  end
  local group_antifuck_lock = data[tostring(target)]['settings']['antifuck']
  if group_antifuck_lock == 'yes' then
    return 'fack is already locked'
  else
    data[tostring(target)]['settings']['antifuck'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'fauck has been locked'
  end
end
local function unlock_group_antifuck(msg, data, target)
  if not is_momod(msg) then
    return "For moderators only!"
  end
  local group_antifuck_lock = data[tostring(target)]['settings']['antifuck']
  if group_antifuck_lock == 'no' then
    return 'fack is already unlocked'
  else
    data[tostring(target)]['settings']['antifuck'] = 'no'
    save_data(_config.moderation.data, data)
    return 'fack has been unlocked'
  end
end
   ...
   if matches[1] == 'lock' or matches[1] == 'l' then
      ...
      if matches[2] == 'all' then
      	...
      	local safemode ={
			...
			lock_group_antifuck(msg, data, target),
      	}
      	return safemode
      end
      ...
      if matches[2] == 'bots' then
        ...
        savelog(msg.to.id, name_log.." ["..msg.from.id.."] link locked ")
        return antilink(msg, data, target)
      end
	  	        if matches[2] == 'antifuck' then
        savelog(msg.to.id, name_log.." ["..msg.from.id.."] fack locked ")
        return antifuck(msg, data, target)
      end
    end
    if matches[1] == 'unlock' or matches[1] == 'u'  then
      ...
      if matches[2] == 'all' then
      	...
      	local de_safemode ={
			...
			unlock_group_antifuck(msg, data, target),
      	}
      	return de_safemode
      end
      if matches[2] == 'sticker' or matches[2] == 's' then
